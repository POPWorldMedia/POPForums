...
public class ProfileRepository : IProfileRepository
{
	...
	public async Task<Profile> GetProfile(int userID)
	{
		...
		await _sqlObjectFactory.GetConnection().UsingAsync(connection => 
			profile = connection.QuerySingleOrDefaultAsync<Profile>("SELECT UserID, IsSubscribed, Signature, ShowDetails, Location, IsPlainText, DOB, Web, Facebook, Instagram, IsTos, AvatarID, ImageID, HideVanity, LastPostID, Points, IsAutoFollowOnReply, IsAutoRenewal, Last4 FROM pf_Profile WHERE UserID = @UserID", new { UserID = userID }));
	...
	{
		await _sqlObjectFactory.GetConnection().UsingAsync(connection => 
			connection.ExecuteAsync("INSERT INTO pf_Profile (UserID, IsSubscribed, Signature, ShowDetails, Location, IsPlainText, DOB, Web, Facebook, Instagram, IsTos, AvatarID, ImageID, HideVanity, LastPostID, Points, IsAutoFollowOnReply, IsAutoRenewal, Last4) VALUES (@UserID, @IsSubscribed, @Signature, @ShowDetails, @Location, @IsPlainText, @DOB, @Web, @Facebook, @Instagram, @IsTos, @AvatarID, @ImageID, @HideVanity, @LastPostID, @Points, @IsAutoFollowOnReply, @IsAutoRenewal, @Last4)", new { profile.UserID, profile.IsSubscribed, Signature = profile.Signature.NullToEmpty(), profile.ShowDetails, Location = profile.Location.NullToEmpty(), profile.IsPlainText, profile.Dob, Web = profile.Web.NullToEmpty(), Instagram =  profile.Instagram.NullToEmpty(), Facebook = profile.Facebook.NullToEmpty(), profile.IsTos, profile.AvatarID, profile.ImageID, profile.HideVanity, profile.LastPostID, profile.Points, profile.IsAutoFollowOnReply, profile.IsAutoRenewal, profile.Last4 }));
	}
	...
	public async Task<bool> Update(Profile profile)
	{
		...
		await _sqlObjectFactory.GetConnection().UsingAsync(connection => 
			success = connection.ExecuteAsync("UPDATE pf_Profile SET IsSubscribed = @IsSubscribed, Signature = @Signature, ShowDetails = @ShowDetails, Location = @Location, IsPlainText = @IsPlainText, DOB = @DOB, Web = @Web, Facebook = @Facebook, Instagram = @Instagram, IsTos = @IsTos, AvatarID = @AvatarID, ImageID = @ImageID, HideVanity = @HideVanity, LastPostID = @LastPostID, Points = @Points, IsAutoFollowOnReply = @IsAutoFollowOnReply, IsAutoRenewal = @IsAutoRenewal, Last4 = @Last4 WHERE UserID = @UserID", new { profile.UserID, profile.IsSubscribed, Signature = profile.Signature.NullToEmpty(), profile.ShowDetails, Location = profile.Location.NullToEmpty(), profile.IsPlainText, profile.Dob, Web = profile.Web.NullToEmpty(), Instagram = profile.Instagram.NullToEmpty(), Facebook = profile.Facebook.NullToEmpty(), IsTos = profile.IsTos, profile.AvatarID, profile.ImageID, profile.HideVanity, profile.LastPostID, profile.Points, profile.IsAutoFollowOnReply, profile.IsAutoRenewal, profile.Last4 }));
