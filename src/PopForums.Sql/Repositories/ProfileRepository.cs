...
public class ProfileRepository : IProfileRepository
{
	...
	public async Task<Profile> GetProfile(int userID)
	{
		...
		await _sqlObjectFactory.GetConnection().UsingAsync(connection => 
			profile = connection.QuerySingleOrDefaultAsync<Profile>("SELECT UserID, IsSubscribed, Signature, ShowDetails, Location, IsPlainText, DOB, Web, Facebook, Twitter, Instagram, IsTos, AvatarID, ImageID, HideVanity, LastPostID, Points FROM pf_Profile WHERE UserID = @UserID", new { UserID = userID }));
	...
	{
		await _sqlObjectFactory.GetConnection().UsingAsync(connection => 
			connection.ExecuteAsync("INSERT INTO pf_Profile (UserID, IsSubscribed, Signature, ShowDetails, Location, IsPlainText, DOB, Web, Facebook, Twitter, Instagram, IsTos, AvatarID, ImageID, HideVanity, LastPostID, Points) VALUES (@UserID, @IsSubscribed, @Signature, @ShowDetails, @Location, @IsPlainText, @DOB, @Web, @Facebook, @Twitter, @Instagram, @IsTos, @AvatarID, @ImageID, @HideVanity, @LastPostID, @Points)", new { profile.UserID, profile.IsSubscribed, Signature = profile.Signature.NullToEmpty(), profile.ShowDetails, Location = profile.Location.NullToEmpty(), profile.IsPlainText, profile.Dob, Web = profile.Web.NullToEmpty(), Instagram =  profile.Instagram.NullToEmpty(), Facebook = profile.Facebook.NullToEmpty(), Twitter = profile.Twitter.NullToEmpty(), profile.IsTos, profile.AvatarID, profile.ImageID, profile.HideVanity, profile.LastPostID, profile.Points }));
	}
