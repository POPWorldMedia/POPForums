...
public class TextParsingService : ITextParsingService
{
	...
	public string Censor(string text)
	{
		...
		{
			for (var j = 1; j <= badWord.Length; j++)
            {
                newWord += _settingsManager.Current.CensorCharacter;
            }
	...
	public string CleanForumCode(string text)
	{
		...
		// handle images
		text = _settingsManager.Current.AllowImages
            ? Regex.Replace(text, @"(\[image){1}(?!\=).+?\]", String.Empty, RegexOptions.IgnoreCase)
            : Regex.Replace(text, @"\[image=.+?\]", String.Empty, RegexOptions.IgnoreCase);
	...
	public string CleanForumCodeToHtml(string text)
	{
		...
		if (!text.StartsWith("[quote]") && !string.IsNullOrWhiteSpace(text))
        {
            text = "<p>" + text;
        }

        if (!text.EndsWith("[/quote]") && !string.IsNullOrWhiteSpace(text))
        {
            text += "</p>";
        }
