...
public class LastReadService : ILastReadService
{
	...
	public async Task GetForumReadStatus(User user, CategorizedForumContainer container)
	{
        ...
        foreach (var forum in container.AllForums)
		{
            ...
            if (lastReads != null && lastReads.ContainsKey(forum.ForumID) && forum.LastPostTime > lastReads[forum.ForumID])
            {
	...
	{
        return user != null ? await _lastReadRepository.GetLastReadTimeForTopic(user.UserID, topic.TopicID) : null;
    }

    public async Task<DateTime?> GetForumReadStatus(User user, Forum forum)
	{
        return user != null ? await _lastReadRepository.GetLastReadTimesForForum(user.UserID, forum.ForumID) : null;
    }
