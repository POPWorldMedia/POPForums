...
public class UserServiceTests
{
	...
	public async Task EditUserProfilePlainText()
	{
		...
		{
			Dob = new DateTime(2000,1,1), HideVanity = true, Instagram = "i", IsPlainText = true, IsSubscribed = true, Location = "l", Facebook = "fb", Twitter = "tw", ShowDetails = true, Signature = "s", Web = "w", IsAutoFollowOnReply = true
		};
	...
	public async Task EditUserProfileRichText()
	{
		...
		_mockProfileRepo.Setup(p => p.GetProfile(1)).ReturnsAsync(returnedProfile);
		_mockProfileRepo.Setup(p => p.Update(It.IsAny<Profile>())).Callback<Profile>(p => profile = p);
		_mockTextParser.Setup(t => t.ClientHtmlToHtml(It.IsAny<string>())).Returns("parsed");
		var userEdit = new UserEditProfile
		{
