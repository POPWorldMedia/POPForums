...
`build.js` compiles TypeScript itself (`tsc -p Client/tsconfig.json`, same as `Client/*.ts` → `wwwroot/PopForums.js`) before minifying — it must be fully self-sufficient standalone, since in CI it's the *only* thing that runs before `dotnet build` (see below). `Microsoft.TypeScript.MSBuild` also compiles TypeScript natively as part of `dotnet build`, independently — the two are redundant when both run, which is why `build.js` skips its own `tsc` step (via the `POPFORUMS_SKIP_TSC` env var) when invoked from the MSBuild target below, since `CompileTypeScript` already ran by then.

`PopForums.Mvc.csproj` wires `build.js` into a custom `BuildFrontendAssets` MSBuild target (`Exec Command="node build.js"`), hooked into `AssignTargetPathsDependsOn`/`DefineStaticWebAssetsDependsOn`/`ResolveStaticWebAssetsInputsDependsOn` the same way `CompileTypeScript` already was (including hot reload — see the `<!--This is an important line...-->` comment) — so on the CLI, in CI, and in any IDE, `dotnet build` re-minifies and re-copies assets automatically, with no editor-specific task-runner binding required. The target has `Inputs`/`Outputs` (`Client/**/*.ts`, `wwwroot/Admin.js`, `wwwroot/*.css`, `package.json` → `wwwroot/lib/PopForums/dist/PopForums.min.js`) so it's skipped as up-to-date on rebuilds that don't touch front-end sources (e.g. a `.cs`-only edit) — without this, every hot-reload-triggered rebuild would re-copy the full vendor asset set (~300 tinymce files) for no reason.
...
Views (`PopForumsMaster.cshtml`, admin `App.cshtml`) use ASP.NET Core's `<environment>` tag helper and **are** gated by environment, contrary to what this doc used to claim: in `Development` they load the raw, unminified `~/PopForums/PopForums.js`/`PopForums.css`/`Admin.js` straight from `wwwroot` root; only outside `Development` do they load the `lib/PopForums/dist/*.min.*` files `build.js` produces. So local dev's "edit a `.ts` file, see it rebuild" loop only ever depended on `CompileTypeScript` regenerating `wwwroot/PopForums.js` — `build.js`/`BuildFrontendAssets` were never in that visible loop; skipping the asset build only breaks non-Development environments (which is what CI/production actually serve), not local dev's visual feedback.
