...
* Clone the latest source code from GitHub, or use the production packages as described above. Build it. Before your first build, run `npm install` and `npm run build` in the `PopForums.Mvc` project to fetch and minify the front-end assets — this needs to happen once before `dotnet build` on a clean checkout; after that, `dotnet build` keeps them in sync automatically.
