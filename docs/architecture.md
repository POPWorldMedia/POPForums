...
All of the web app code is contained in the `PopForums.Mvc` project, which references the base `PopForums` library. The host app (`PopForums.Web`) then brings in `PopForums.Sql` and the above-mentioned scale-out libraries as necessary. The assets, including all of the CSS and transpiled TypeScript, are also shipped in the `Mvc` library. This makes it really easy to update the forum bits in your own application, without having to replace a bunch of loose files. Update the package, and you're done.
